...
class MicroServer {
  ...
  #startHttpServer() {
    ...
    let routerPath='/:service/:logic/:func';
    if(this.config.static?.opts?.apiPrefix!==null){
      routerPath=this.config.static.opts.apiPrefix+routerPath;
    }
    ...
    if (this.config.static.enabled) {
      ...
      if(Object.keys(opts).length>0){
        app.use(async (ctx,next) => {
          if (ctx.method === 'GET' && !ctx.path.startsWith(opts.apiPrefix) && ctx.status === 404) {
            // Optionally add logic to exclude API or asset paths
            ctx.type = 'html';
            ctx.body = await fs.readFile(path.join(this.config.projectDir,this.config.static.dirName, opts.index), 'utf8');
            ctx.status = 200;
          }else{
            await next()
          }
    ...
    if (this.config.sse.enabled) {
      ...
      app.use(
        async (ctx,next)=>{
          console.log('path to sse: ',ctx.path);
          if(this.config.sse.config?.matchPath && ctx.path===this.config.sse.config?.matchPath){
            const sseHandler=require(path.join(this.config.projectDir, this.config.sse.filePath));
            await sseHandler(ctx);
          }else{
            await next();
          }
        }
