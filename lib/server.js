...
class MicroServer {
  ...
  #startHttpServer() {
    ...
    if(this.config.sse.enabled){
      ...
      app.use(async(ctx)=>{
        try {
          const sseHandlers = require(path.join(
            this.config.projectDir,
            this.config.sse.filePath
          ));
          // map them to route based on keys and values
          for (const path of Object.keys(sseHandlers)) {
            // console.log(`loading sse handler for path: ${path}`);
            sseHandlers[path](ctx);
          }
        } catch (err) {
          console.log("sse loaded failed, error: ", err);
        }
      });
    }
