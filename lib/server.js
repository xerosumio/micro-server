...
class MicroServer {
  ...
  async #callFunc(service, logic, func, reqBody) {
    console.log(
      `service: ${service}, logic: ${logic}, func: ${func}, reqBody: ${JSON.stringify(
        reqBody
      )}`
    );
  ...
  #startHttpServer() {
    ...
    // Move SSE middleware before static
    ...
    if (this.config.static.enabled) {
      ...
      app.use(
        serve(
          path.join(this.config.projectDir, this.config.static.dirName),
          opts
        )
      );
    ...
    // Move SSE middleware here, after static, before multer/router
    // if (this.config.sse.enabled) {
    //   // Only handle SSE on the configured matchPath
    //   const sseMatchPath = this.config.sse?.opts?.matchPath || "/events";
    //   app.use(async (ctx, next) => {
  ...
  // ...SSE middleware now handled above...
