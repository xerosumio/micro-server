  ...
  let routerPath = `${apiPrefix}/:service/:logic/:func`;
    ...
    if (this.config.static.enabled) {
      ...
      app.use(async (ctx, next) => {
        ...
        console.log(`fs.existsSync(path.join(this.config.projectDir, this.config.static.dirName, opts.index)): ${fs.existsSync(path.join(this.config.projectDir, this.config.static.dirName, opts.index))}`)
        if (
          ctx.method === 'GET' &&
          !isApi &&
          !isSse &&
