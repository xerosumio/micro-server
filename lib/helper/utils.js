 ...
 */
const matchPasswordRule = (ruleRegExp,password, errorCode = 102) => {
  if (!ruleRegExp.test(password)) {
      const err = new Error(
          "password must be numbers, uppercase/lowercase letters, between 6 and 16 in length, (at least one uppercase letter)"
      );
      err.code = errorCode;
      throw err;
  }
