...
const compose = (...fns) => {
  return async (_reqObj) => {
    const reqObj = { ..._reqObj, state: {},passdown:{} };
    let result;
    for (const fn of fns) {
...
function checkObjQualify(obj,key_ary){
  return JSON.stringify(Object.keys(obj))===JSON.stringify(key_ary);
}

/**
 * This passed a readable stream created using fs.createReadStream
 * mainly to handle the uploaded files
 * 
 * @param {ReadStream} readerStream stream created using fs.createReadStream which opens a stream to read the files uploaded, they usually in binary formats
 * @param {string} filePath file path that the writer stream would be written to. The writerStream will be created based on this filepath
 */
async function storeUploadedFile(readerStream,filePath){
  const writer=fs.createWriteStream(filePath);
  readerStream.pipe(writer);
  return [
