...
class SQLConnector {
  ...
  */
  ...
  async count(table,target_col=null,is_distinct=false){
    const connection = await this.#pool.getConnection();
    const [result] = await connection.query(`SELECT COUNT(${is_distinct ? 'DISTINCT' : ''}${target_col ? target_col : '*'}) as count FROM ${table}`);
    await connection.release();
    return result.count;
  }
}
