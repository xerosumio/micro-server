...
class SQLConnector{
  ...
  async createOne(table,object){
    ...
    const [result]=await connection.query(`INSERT INTO ${table} SET ?`,object);
  ...
  async read(table,filter=null,limit=null){
    ...
    // return result;
    const connection=await this.#pool.getConnection();
    const [result]=await connection.query(`SELECT * FROM ${table} ${filter? `WHERE ${filter}`:''} ${limit? `LIMIT ${limit}`:''}`);
    await connection.release();
    return result;
  }
  ...
  async updateById(table,object,id){
    const connection=await this.#pool.getConnection();
    const [result]=await connection.query(`UPDATE ${table} SET? WHERE id =?`,[object,id]);
    await connection.end();
    return result.affectedRows;
  }
