...
class SQLConnector{
  ...
  async read(table,filter=null,limit=null){
    const connection=await this.#pool.getConnection();
    let values=[];
    filter ? values.push(filter) : null;
    limit ? values.push(limit) : null;
    const [result]=await connection.query(`SELECT * FROM ${table} ${filter? `WHERE ?`:``} ${limit? `LIMIT ?`:''}`,values);
    await connection.release();
    return result;
  }
  ...
  async readOne(table,filter=null){
    ...
    const [result]=await connection.query(`SELECT * FROM ${table} ${filter? `WHERE ?`:``} LIMIT 1`,value);
    await connection.release();
    return result[0];
  }
