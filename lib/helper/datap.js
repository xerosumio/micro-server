...
class SQLConnector {
  ...
  */
  ...
  async readByCustomSQLString(string,values){
    const connection = await this.#pool.getConnection();
    const [result]=await connection.query(string,values ? values : null);
    await connection.release();
    return result;
  }
  ...
  async count(table,target_col=null,is_distinct=false,aliase=''){
    const connection = await this.#pool.getConnection();
    const [result] = await connection.query(`SELECT COUNT(${is_distinct ? 'DISTINCT' : ''}${target_col ? target_col : '*'}) ${aliase!==''? `as ${aliase}`:''} FROM ${table}`);
    await connection.release();
    if(aliase!==''){
      return result[0]
    }else{
