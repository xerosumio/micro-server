...
class SQLConnector{
  ...
  async createOne(table,object){
    const [result]=await this.#connection.query(`INSERT INTO ${table} SET ?`,object);
  ...
  async read(table,filter=null,limit=null){
    // may have preprocessing for constructing query string for filter
    // it is default the filter object is AND condition
    // ${filter? `WHERE ${filter}`:''}
    const [result]=await this.#connection.query(`SELECT * FROM '${table}' ${filter? `WHERE ${filter}`:''} ${limit? `LIMIT ${limit}`:''}`);
    await this.#connection.end();
    return result;
  }
