...
class SQLConnector {
  ...
  async uploadimg(table, file, img_col) {
    const connection = await this.#pool.getConnection();
    const [result] = await connection.query(
      `INSERT INTO ${table} (${img_col}) VALUES (BINARY(?))`,
      file
    );
    await connection.release();
    return result.insertId;
   ...
   * 
   * @param {string} table 
   * @param {Array<any>} objects an array nested with the values of the data, the element sequence should follow the cols
   * @param {Array<string>} cols indicates which column data would be insert
   * @returns 
   */
  async createmany(table, objects, cols = null) {
    const connection = await this.#pool.getConnection();
    const [result] = await connection.query(`INSERT INTO ${table} ${cols ? `(${cols.join(',')})` : ``} VALUES ?`, objects);
    await connection.release();
  ...
  */
  ...
  async read(table, filter = null, limit = null, order = null) {
    ...
    if (!(filter === null || filter === undefined)) {
      // if the filter does not have the toSqlString function, pass it to the cusome object constructor
      if (filter?.toSqlString === undefined || filter?.toSqlString === null) {
        const obj = this.#constructSqlStringObj(filter);
        values.push(obj);
        // else, directly deal with it
      } else {
        values.push(filter);
      }
    }
    if (!(order === null || order === undefined)) {
      if(order?.toSqlString===undefined || order?.toSqlString===null){
  ...
  async readByCustomSQLString(string, query) {
    const connection = await this.#pool.getConnection();
    const [result] = await connection.query(string, query ? query : null);
    await connection.release();
    return result;
  }
  ...
  async deletemany(table, filter) {
    ...
    const result = await connection.query(`DELETE FROM ${table} ${filter ? `WHERE ?` : ``}`, values);
    await connection.release();
    return result.affectedRows;
  }
  ...
  async updatemany(table, object, filter, order, limit) {
    const connection = await this.#pool.getConnection();
    let values = [];
    values.push(object);
    if (!(filter === undefined || filter === null)) {
    ...
    limit ? values.push(limit) : null;
    const [result] = await connection.query(`UPDATE ${table} SET ? ${filter ? `WHERE ?` : ``} ${order ? `ORDER BY ?` : ""}
      ${limit ? `LIMIT ?` : ""}`, values);
    await connection.release();
    return result.affectedRows;
  }
  ...
  async count(
    ...
    if (!(query === null || query === undefined)) {
      if (Object.keys(query).indexOf('toSqlString') < 0) {
        obj = this.#constructSqlStringObj(query);
        value.push(obj);
      } else {
        value.push(query);
      }
    ...
    const [result] = await connection.query(
      `SELECT COUNT(${is_distinct ? "DISTINCT " : ""}${target_col ? target_col : "*"
      }) ${aliase !== "" ? `as ${aliase}` : ""} FROM ${table} ${query ? `WHERE ?` : ""
      }`,
      [value]
    );
    await connection.release();
    if (aliase !== "") {
      return result[0][aliase];
    } else {
      return result[0];
    }
  }
