...
class SQLConnector{
  ...
  async createOne(table,object){
    const {result}=this.#connection.query(`INSERT INTO ${table} VALUES(?)`,object);
  ...
  async read(table,filter=null,limit=10){
    ...
    const {result}=await this.#connection.query(`SELECT * FROM '${table}' ${filter? `WHERE ${filter}`:''} LIMIT ${limit}`);
    await this.#connection.end();
    return result;
  }

  async readOne(table,filter=null,limit=10){
    const {result}=await this.#connection.query(`SELECT * FROM ${table} ${filter? `WHERE ${filter}`:''} LIMIT ${limit}`);
    await this.#connection.end();
    return result;
  }
