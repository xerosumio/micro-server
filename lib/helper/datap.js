...
class SQLConnector{
  ...
  async connect(object){
    if(object?.host===undefined || object?.database===undefined || object?.user===undefined || object?.password===undefined){
      const err=new Error('missing fields for establising sql db connection.');
      err.code=500;
      throw err;
    }
    ...
    this.#db=object.database;

    this.#connection = await mysql.createConnection(object);
  }

  async createOne(table,objectAry,fields=null){
    const field=fields?`(${fields.join(',')})`:'';
    const [result]=await this.#connection.query(`INSERT INTO ${table} ${field} VALUES(?)`,objectAry);
  ...
  async read(table,filter=null,limit=10){
    // may have preprocessing for constructing query string for filter
    // it is default the filter object is AND condition
    // ${filter? `WHERE ${filter}`:''}
    const [result]=await this.#connection.query(`SELECT * FROM '${table}' LIMIT ${limit}`);
    await this.#connection.end();
    return result;
  }

  async readId(table,id){
    const [result]=await this.#connection.query(`SELECT * FROM ${table} WHERE id =?`,id);
    await this.#connection.end();
    return result;
  }

  async deleteId(table,id){
    const [result]=await this.#connection.query(`DELETE FROM ${table} WHERE id =?`,id);
    await this.#connection.end();
    return result.affectedRows;
  }

  async deleteMany(table,objects){}

  async updateById(table,object,id){
    const [result]=await this.#connection.query(`UPDATE ${table} SET? WHERE id =?`,[object,id]);
    await this.#connection.end();
    return result.affectedRows;
  }
